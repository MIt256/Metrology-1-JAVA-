...
begin
  ...
  for i := 0 to length(lexems) - 1 do
  begin
    if (lexem_in_java_statements(lexems[i])) or (lexem_in_java_keywords(lexems[i])) then
...
begin
  ...
  begin
   // line_str - выделенная лексема
    while line_str[lexem_pos] = ' ' do
      lexem_pos := lexem_pos + 1;

    if ((line_str[1]= '/') and (line_str[2] = '/' ))then //однострочный комментарий  - выход
    begin  //можно запилить и для многострочных, но там проверка криво работала
    //да и Болтак это не нужно :)
    //Если и скажет закомментировать, то комментите однострочными. Нечего выебываться.
      Break;
    end;

    i := lexem_pos;

    while (line_str[i] in java_identifier) do
      i := i + 1;
    ...
    if i = lexem_pos then
      while lexem_in_java_statements(line_str[i]) do
        i := i + 1;

    lexem := copy(line_str, lexem_pos, i - lexem_pos);

    if ((line_str[i] = '(')) and (not (lexem_in_java_statements(lexem))) then //если функция или оператор-СКОБКА
    begin
      lexem := lexem + '( )';
      i := i + 1;
    end;
    ...
    if ((lexem <> '') ) then
      if (not(lexem_in_java_not_in(lexem))) then //если символ/лексема не определены - ничего не добавляется
    begin
      SetLength(Result, length(Result) + 1);
